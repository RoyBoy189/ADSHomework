...
class DSHashMap<V> implements Iterable<String> {
     ...
     */
    V get(String v1) {
        int location = hash(v1);
        if (a.get(location) == null) {
            throw new IndexOutOfBoundsException("Key not found: " + v1);
        } else { // Already a chain here, with stuff in it
            // search that chain for our key.
            for (int i = 0; i < a.get(location).length(); i++) {
                KVP kvp = a.get(location).get(i);
     ...
     * 
     * @param v1 The key to search for
     * @return True if the key already exists in the hash map
     */
    public Boolean containsKey(String v1) {
        int location = hash(v1);
        if (a.get(location) == null) {
            return false;
        } else { // Already a chain here, with stuff in it
    ...
    public int longestChain() {
        ...
        for (int x = 0; x < a.capacity(); x++) {
            if (a.get(x) != null)
                if (a.get(x).length > maxChainLength) {
                    maxChainLength = a.get(x).length;
                }
        }
    ...
    public DSArrayList<String> keysForValue(V value) {
        ...
        DSArrayList<String> olda = new DSArrayList<String>();
        for (int i = 0; i < a.capacity(); i++) {          
                if (a.get(i) != null) {
                    for (int n = 0; n < a.get(i).length; n++) {
                    if(a.get(i).get(n).value.equals(value)) {
                    olda.add(a.get(i).get(n).key);    
                    }               
               }
           }
        }
    ...
    @Override
     public String toString() {
         ...
         String eachkey = "{";
         DSArrayList<String> olda = new DSArrayList<String>();
         // V eachvalue = a.get(0).value;
        for (int i = 0; i < a.capacity(); i++) {
                 if (a.get(i) != null) {
                     for (int n = 0; n < a.get(i).length; n++) {
             eachkey += a.get(i).get(n).key + ":" + a.get(i).get(n).value + ", ";    
                 }
             }   
         }  
         return eachkey + "}";
    ...
    public Iterator<String> iterator() {
        return new Iterator<String>() {
            ...
            public boolean hasNext() {
                ...
                backindex++;
                while (backindex < capacity && a.get(backindex) == null)
                    backindex++;
